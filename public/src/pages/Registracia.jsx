...
import { registraciaRoute } from "../utils/apiRoutes.js";
...
export default function Registracia() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (handleValidation()) {
      ...
      const { heslo, uzivatelske_meno, email } = values;
  ...
  const handleValidation = () => {
    const { heslo, potvrditHeslo, uzivatelske_meno, email } = values;

    if (heslo !== potvrditHeslo) {
      toast.error("Heslo a potvrdené heslo sa musia zhodovať!", toastOptions);
      return false;
    } else if (uzivatelske_meno.length < 3) {
      toast.error(
      ...
      toast.error(
        "Heslo musí byť rovné alebo dlhšie ako 8 znakov!",
        toastOptions
      );
      return false;
    } else if (email === "") {
      toast.error("E-mail je povinný!", toastOptions);
      return false;
    }
    return true;
  };
    ...
    <>
      <FormContainer>
        <form onSubmit={(event) => handleSubmit(event)}>
          ...
          <span>
            Už máš účet? <Link to="/prihlasenie"> Prihlásiť Sa </Link>
          </span>
        </form>
      </FormContainer>
      <ToastContainer />
